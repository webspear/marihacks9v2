...
x = np.array([[0.],[-80000000000000000000000000.],[60000.]])
y = np.array([[0.],[0.],[5000.]])
m = np.array([1000000000,100000,100000])
r = np.array([32,16,16])

v = np.array([[30.,80.],[10.,4000.],[-20.,5000.]])
...
def init():
    ax.set_xlim(x[0][0]-size, x[0][0]+size)
    ax.set_ylim(y[0][0]-size, y[0][0]+size)
...
global plot1
global plot2
plot0, = ax.plot(x[0], y[0], 'ro')
plot0.set_color("black")
plot0.set_markersize(16)
plot1, = ax.plot(x[1], y[1], lw=2)
plot1.set_color("blue")
plot2, = ax.plot(x[2], y[2], lw=2)
plot2.set_color("blue")
ax.set_xlabel('x')
ax.set_ylabel('y')
ax.set_title("2-body problem | Star & Planet")
ax.legend(['Star', 'Planet'])

axX0 = fig.add_axes([0.25, 0.2, .07, 0.03])
x0 = Slider(
    ax=axX0,
    label='X0',
    valmin=-15000,
...
axX1 = fig.add_axes([0.55, 0.2, .07, 0.03])
x1 = Slider(
    ax=axX1,
    label='X1',
    valmin=-15000,
...
axY0 = fig.add_axes([0.4, 0.2, .07, 0.03])
y0 = Slider(
    ax=axY0,
    label='Y0',
    valmin=-15000,
...
axY1 = fig.add_axes([0.7, 0.2, .07, 0.03])
y1 = Slider(
    ax=axY1,
    label='Y1',
    valmin=-15000,
...
axXV0 = fig.add_axes([0.25, 0.17, .07, 0.03])
Vx0 = Slider(
    ax=axXV0,
    label='Vy0',
    valmin=-256,
...
axXV1 = fig.add_axes([0.55, 0.17, .07, 0.03])
Vx1 = Slider(
    ax=axXV1,
    label='Vy1',
    valmin=-256,
...
axYV0 = fig.add_axes([0.4, 0.17, .07, 0.03])
Vy0 = Slider(
    ax=axYV0,
    label='Vy0',
    valmin=-256,
...
axYV1 = fig.add_axes([0.7, 0.17, .07, 0.03])
Vy1 = Slider(
    ax=axYV1,
    label='Vy1',
    valmin=-256,
...
axM0 = fig.add_axes([0.32, 0.14, .07, 0.03])
M0 = Slider(
    ax=axM0,
    label='M0',
    valmin=1000000,
...
axM1 = fig.add_axes([0.62, 0.14, .07, 0.03])
M1 = Slider(
    ax=axM1,
    label='M1',
    valmin=10000,
...
def update(i):
    ...
    for i in range(16):
        ...
        ax.set_xlim(x[0][-1]-size, x[0][-1]+size)
        ax.set_ylim(y[0][-1]-size, y[0][-1]+size)
        return plot0,
...
def updateVal(val):
    ...
    x = np.array([[x0.val],[10000000000000000000],[x1.val]])
    y = np.array([[y0.val],[10000000000000000000],[y1.val]])
    m = np.array([M0.val,M1.val,M1.val])

    v = np.array([[Vx0.val,Vy0.val],[Vx1.val,Vy1.val],[Vx1.val,Vy1.val]])
