...
def EulersMethod():
    TEMP = G*m[0]*m[1]/(pow(x[0]-x[1],2) + pow(y[0]-y[1],2))
    vec = normalVecTowards([x[0],y[0]],[x[1],y[1]])
    v[0][0] += (TEMP/m[0]*delta)*vec[0]
    v[0][1] += (TEMP/m[0]*delta)*vec[1]
    vec = normalVecTowards([x[1],y[1]],[x[0],y[0]])
    v[1][0] += (TEMP/m[1]*delta)*vec[0]
    v[1][1] += (TEMP/m[1]*delta)*vec[1]
    
    TEMP = G*m[0]*m[2]/(pow(x[0]-x[2],2) + pow(y[0]-y[2],2))
    vec = normalVecTowards([x[0],y[0]],[x[2],y[2]])
    v[0][0] += (TEMP/m[0]*delta)*vec[0]
    v[0][1] += (TEMP/m[0]*delta)*vec[1]
    vec = normalVecTowards([x[2],y[2]],[x[0],y[0]])
    v[2][0] += (TEMP/m[2]*delta)*vec[0]
    v[2][1] += (TEMP/m[2]*delta)*vec[1]
    
    TEMP = G*m[1]*m[2]/(pow(x[1]-x[2],2) + pow(y[1]-y[2],2))
    vec = normalVecTowards([x[1],y[1]],[x[2],y[2]])
    v[1][0] += (TEMP/m[1]*delta)*vec[0]
    v[1][1] += (TEMP/m[1]*delta)*vec[1]
    vec = normalVecTowards([x[2],y[2]],[x[1],y[1]])
    v[2][0] += (TEMP/m[2]*delta)*vec[0]
    v[2][1] += (TEMP/m[2]*delta)*vec[1]
...
def init():
    ax.set_xlim(x[0]-20000, x[0]+20000)
    ax.set_ylim(y[0]-20000, y[0]+20000)
